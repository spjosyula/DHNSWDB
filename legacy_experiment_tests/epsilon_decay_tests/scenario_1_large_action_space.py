"""Scenario 1: Large Action Space Test

Test epsilon decay effectiveness with 20 ef_search candidates instead of 6.

Hypothesis:
    Larger action space requires longer exploration phase. GLIE decay should:
    1. Explore all 20 actions more systematically early on
    2. Converge to optimal actions faster than fixed epsilon
    3. Show 3-5% efficiency improvement

Configuration:
    - ef_candidates: [20, 30, 40, 50, 60, 75, 90, 100, 110, 125, 140, 150, 165, 180, 200, 220, 250, 275, 300, 350]
    - k_intents: 3 (same as baseline)
    - num_queries: 500 (need more queries for larger space)
    - Control: Fixed epsilon = 0.15
    - Treatment: GLIE epsilon decay starting at 0.4

Expected Outcome:
    GLIE should show 3-5% efficiency improvement due to better exploration-exploitation balance
    in the larger action space.
"""
...
def run_experiment(
    ...
    """Run experiment with large action space.
    ...
    # Override ef_selector with large action space
    ...
    # Run queries
    tracker = ExperimentTracker(f"{epsilon_decay_mode}_eps{initial_epsilon}_large_action_space")
    ...
    print(f"\nAction distribution (ef_search values used):")
    ef_counts = {}
...
def main():
    """Run Scenario 1: Large Action Space."""
    print("\n" + "=" * 100)
    print("SCENARIO 1: LARGE ACTION SPACE TEST")
    print("=" * 100)
    print("\nHypothesis:")
    print("  Larger action space (20 ef candidates) requires more systematic exploration.")
    print("  GLIE decay should show 3-5% efficiency improvement over fixed epsilon.")
    print("\nConfiguration:")
    print(f"  Action space: {len(LARGE_EF_CANDIDATES)} ef candidates")
    print(f"  ef_candidates: {LARGE_EF_CANDIDATES}")
    print(f"  k_intents: 3")
    print(f"  num_queries: 500")
    print("\nComparison:")
    ...
    control_tracker.save_results("epsilon_decay_tests/results/scenario_1_control.json")
    treatment_tracker.save_results("epsilon_decay_tests/results/scenario_1_treatment.json")
    print("  Saved to: epsilon_decay_tests/results/scenario_1_*.json")
    print("=" * 100)
