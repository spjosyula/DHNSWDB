"""Scenario 2: High-Dimensional Embeddings Test

Test adaptive ef_search with higher-dimensional embeddings where search cost is higher.

Hypothesis:
    High-dimensional spaces (768 dims) make ef_search selection more critical:
    1. Higher dimensions = more expensive distance calculations
    2. Efficiency gains from lower ef_search are more pronounced
    3. Adaptive learning can find sweet spot between speed and recall
    4. Should show 8-12% efficiency improvement over static

Configuration:
    - Corpus size: 3000 documents
    - Embedding: sentence-transformers all-mpnet-base-v2 (768 dims, better quality)
    - Queries: 800 mixed (exploratory, precise, mixed intents)
    - k_intents: 4
    - ef_candidates: [30, 50, 75, 100, 125, 150, 200]
    - Static baseline: ef_search = 100
    - Adaptive: Q-learning with feedback

Expected Outcome:
    Adaptive should show 8-12% efficiency improvement. Higher dimensional space
    makes ef_search optimization more valuable due to increased computational cost.
"""
...
def run_static_baseline(
    ...
    store = VectorStore(
        dimension=768,  # Higher dimensional
        M=16,
        ef_construction=200,
        ef_search=ef_search,
    ...
    tracker = ExperimentTracker(f"static_ef{ef_search}_768d")
...
def run_adaptive_learning(
    ...
    tracker = ExperimentTracker(f"adaptive_k{k_intents}_768d")

    print(f"Running {len(queries)} queries with adaptive learning...")
    q_snapshot_interval = 80
...
def main():
    """Run Scenario 2: High-Dimensional Embeddings."""
    print("\n" + "=" * 100)
    print("SCENARIO 2: HIGH-DIMENSIONAL EMBEDDINGS TEST")
    print("=" * 100)
    print("\nHypothesis:")
    print("  Higher dimensions (768) increase computation cost, making ef_search optimization")
    print("  more valuable. Adaptive should show 8-12% efficiency improvement.")
    print("\nConfiguration:")
    print(f"  Corpus size: 3000 documents")
    print(f"  Embedding model: sentence-transformers all-mpnet-base-v2 (768 dims)")
    print(f"  Queries: 800 (exploratory, precise, mixed)")
    print(f"  k_intents: 4")
    ...
    corpus = generate_realistic_corpus(size=3000)
    print(f"  Generated {len(corpus)} documents")

    # Embed corpus with higher-dimensional model
    print("\nEmbedding corpus with all-mpnet-base-v2 (768 dims)...")
    ...
    queries, query_types = create_realistic_queries(
        exploratory_count=160,
        precise_count=320,
        mixed_count=320,
    )
    queries = queries[:800]
    query_types = query_types[:800]
    ...
    static_tracker.save_results("adaptive_ef_tests/results/scenario_2_static.json")
    adaptive_tracker.save_results("adaptive_ef_tests/results/scenario_2_adaptive.json")
    print("  Saved to: adaptive_ef_tests/results/scenario_2_*.json")
    print("=" * 100)
